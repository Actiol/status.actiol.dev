...
(function setupServers() {
    ...
    rasp.addService(new Service("mentorship-helper", "https://mentorship.actiol.dev/health"));
    rasp.addService(new Service("honig-bot"));
    rasp.addService(new Service("audio-bot"));
    rasp.addService(new Service("zipline", "https://i.actiol.dev/api/healthcheck"));
    rasp.addService(new Service("fileserver quantum", "https://fs.actiol.dev/health"));
    servers.push(rasp);
})();
(function injectServers() {
    ...
    servers.forEach(async (server) => {
        ...
        const statusPairs = await Promise.all(server.services.map(async (s) => [s, await s.healthCheck()]));
        const mostCommonCode = statusPairs
            .map(([_, status]) => status?.code)
            .filter((code) => !!code)
            .reduce((acc, code, _, arr) => {
            const count = (c) => arr.filter(x => x === c).length;
